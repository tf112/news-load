...
class User(Base):
    ...
    gender: Mapped[Optional[str]] = mapped_column(
        Enum("male", "female", "unknown"), nullable=True, comment="性别"
    )
...
class UserToken(Base):
    ...
    expires_at: Mapped[datetime] = mapped_column(
        DateTime, nullable=False, comment="过期时间", onupdate=datetime.now
    )
    ...
    def __repr__(self):
        return (
            f"UserToken(id={self.id!r}, user_id={self.user_id!r}, token={self.token!r})"
        )
